...
"""
Take two paired end read files, and a file of index reads. Add the last four bases of each read to the sequence id.
"""
...
for reads[0], reads[1], index in izip(SeqIO.parse(gzip.open(sys.argv[1]),"fastq"), \
	...
	for i in range(2):
		reads[i].id += "_" + str(index.seq[7:])
		reads[i].description = reads[i].description.split()[-1]
